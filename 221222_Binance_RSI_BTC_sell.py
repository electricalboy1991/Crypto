...
def update_variable(update, context):
    ...
    update.message.reply_text(f"RSI2_on has been updated to {RSI_on}")

# Function to handle regular messages
def message_handler(update, context):
    message_text = update.message.text
    if message_text.startswith('/r2on'):
        update_variable(update, context)
    else:
        update.message.reply_text("Invalid command")

# Register the command and message handlers
dispatcher.add_handler(CommandHandler('r2on', update_variable))
