...
if amt == 0:
    ...
    if ma5 > ma20 and ma5_before3 < ma5_before2 and ma5_before2 > ma5 and rsi14 >= 35.0:
        ...
        data = binanceX.create_market_sell_order(Target_Coin_Ticker, first_amount)
        #이렇게 주문 데이터를 받으면 data['price']에 현 주문의 평단이 들어있으니 이걸 활용하면 정확한 익절라인을 그을 수 있습니다.
        print(data)
        ...
        #롱 포지션을 걸어 수익화 라인을 그어준다  / data['price'] 활용
        #print(binanceX.create_limit_buy_order(Target_Coin_Ticker, first_amount, coin_price * (1.0 - target_rate)))
        print(binanceX.create_limit_buy_order(Target_Coin_Ticker, first_amount, data['price'] * (1.0 - target_rate)))
    ...
    if ma5 < ma20 and ma5_before3 > ma5_before2 and ma5_before2 < ma5 and rsi14 <= 65.0:
        ...
        data = binanceX.create_market_buy_order(Target_Coin_Ticker, first_amount)
        #이렇게 주문 데이터를 받으면 data['price']에 현 주문의 평단이 들어있으니 이걸 활용하면 정확한 익절라인을 그을 수 있습니다.
        print(data)
        ...
        #숏 포지션을 걸어 수익화 라인을 그어준다 / data['price'] 활용
        #print(binanceX.create_limit_sell_order(Target_Coin_Ticker, first_amount, coin_price * (1.0 + target_rate)))
        print(binanceX.create_limit_sell_order(Target_Coin_Ticker, first_amount, data['price'] * (1.0 + target_rate)))
    ...
    if amt < 0:
        ...
        if ma5 < ma20 and ma5_before3 > ma5_before2 and ma5_before2 < ma5:
            ...
            if revenue_rate >= target_revenue_rate:
                ...
                #롱 포지션을 잡는다 - 지정가는 100% 잡히거나 종료된다는 보장이 없기에 이렇게 시장가를 권장합니다!
        ...
        if ma5 > ma20 and ma5_before3 < ma5_before2 and ma5_before2 > ma5:
            ...
            if revenue_rate <= water_rate and Max_Amount >= abs_amt + water_amount:
                ...
                #숏 포지션을 잡는다 - 지정가는 100% 잡히거나 종료된다는 보장이 없기에 이렇게 시장가를 권장합니다!
