...
TotalMoeny = myUpbit.GetTotalMoney(balances)/5 #총 원금
    ...
    try: 
        ...
        if myUpbit.IsHasCoin(balances,ticker) == True:
            ...
            if won < WaterEnterMoeny and revenu_rate <= -10.0:
                ...
                #시장가 매도를 한다.
                balances = myUpbit.SellCoinMarket(upbit,ticker,upbit.get_balance(ticker) / 2.0)
                line_alert.SendMessage("[업빗_현물] " + str(ticker) + "를 절반 팔고 물타자")
            ...
            if rsi_min_before <= 30.0 and rsi_min > 30.0:
                ...
                if Total_Rate <= 50.0:
                    ...
                    line_alert.SendMessage( "[업빗_현물] 최대매수금액 기준 50% 이하에서 " +str(ticker) + "를 물탐. RSI 업 사이클임.")
                ...
                #50%를 초과하면
                    ...
                    if revenu_rate <= -5.0:
                        ...
                        myUpbit.CancelCoinOrder(upbit,ticker)
                         ...
                         #시장가 매수를 한다.
                        balances = myUpbit.BuyCoinMarket(upbit,ticker,WaterEnterMoeny)
                        line_alert.SendMessage( "[업빗_현물] 최대매수금액 기준 50% 이상에서 " +str(ticker) + "를 물탐. RSI 업 사이클임. & 수익률이 -5% 넘었어.")
        ...
        #아직 매수안한 코인
            ...
            if rsi_min_before <= 30.0 and rsi_min > 30.0 and myUpbit.GetHasCoinCnt(balances) < MaxCoinCnt :
                ...
                line_alert.SendMessage("[업빗_현물] " + str(ticker) + " 첫 매수 진행")
