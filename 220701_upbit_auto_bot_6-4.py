...
def SetStopLoss(binance, Ticker, cut_rate):
    ...
    if StopLossOk == False:
        ...
        #예를 들어, cut rate 이 0.5고 leverage 가 2배로 계산하면, danger rate이 25가 나와
        danger_rate = ((100.0 / leverage) * cut_rate) * 1.0

        #롱일 경우의 손절 가격을 정한다. ->위의 예시를 그대로 가지고 오면, stop price가 entry price의 0.75배로 나옴...
        #그러니까 여기서 손절은 내 원금 기준 손실을 기반으로 코딩한거네/
...
if amt == 0:
    ...
    if ma5 > ma20 and ma5_before3 < ma5_before2 and ma5_before2 > ma5 and rsi14 >= 35.0:
        ...
        #해당 코인 가격을 가져온다.
        #해당 코인 가격을 한번 더 가지고 옴으로서, 지정가 주문이지만 시장가 주문인것처럼 동작하게 한다. 체결 확률을 높이는 거지.
        coin_price = GetCoinNowPrice(binanceX, Target_Coin_Ticker)
...
#0이 아니라면 포지션 잡은 상태
    ...
    #손절 마이너스 수익율을 셋팅한다. // 실제론 아래처럼 레버리지를 곱해야함. 그게 내가 진짜 잃은 돈
    danger_rate = -5.0
    ...
    #water_rate은 수익률과 비교할 값임. 수익률과 water_rate을 비교하게 되면,
    water_rate = -1.0
    ...
    if amt < 0:
        ...
        if ma5 < ma20 and ma5_before3 > ma5_before2 and ma5_before2 < ma5:
            ...
            if revenue_rate >= target_revenue_rate:
                ...
                #롱 포지션을 잡는다
                #abs_amt 는 포지션 정리하는 거고, first_amount 로 다시 이제 실제로 롱 포지션 잡아야지~// 포지션 종료 시키고 바로 새 포지션 잡는 거임..
        ...
        if ma5 > ma20 and ma5_before3 < ma5_before2 and ma5_before2 > ma5:
            ...
            #이미 전체 금액에 80%까지 들어와서, 나머지 20을 물탈 수량으로 정하는 코드
    ...
    #양수면 롱 포지션 상태
        ...
        #즉 5일 선이 20일 선 아래에 있고 상승추세로 꺾였을 때 물을 탑니다. 오를 때 마다 물을 타는 건 별로 스마트 하지 않아보임.
        #롱 포지션을 잡을만한 상황!!!!
        if ma5 < ma20 and ma5_before3 > ma5_before2 and ma5_before2 < ma5:
            #물탈 수량 
            water_amount = abs_amt
