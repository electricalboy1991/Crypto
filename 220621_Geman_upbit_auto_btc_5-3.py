...
access = "mpCGRiTCqlza4eKywK6AIikXQ9ABX3dqQkyuTc3w"
secret = "megYxQfiLpuII681da1vlX2bC9EcGXI0iyg6odSQ"
...
#내가 매수한 (가지고 있는) 코인 개수를 리턴하는 함수// 매수 코인 갯수 제한하기 위함임
def GetHasCoinCnt(balances):
    CoinCnt = 0
    for value in balances:
        avg_buy_price = float(value['avg_buy_price'])
        if avg_buy_price != 0: #원화, 드랍받은 코인(평균매입단가가 0이다) 제외! 아이디어 굿...!!
            CoinCnt += 1
    return CoinCnt
