        ...
        if myUpbit.CheckCoinInList(Kimplist,ticker_upbit) == True:
            ...
            if myUpbit.IsHasCoin(balance_upbit, ticker_upbit) == True:
                ...
                if Krate > 0.5 \
                    ...
                    # data = binanceX.create_market_sell_order(Target_Coin_Ticker,Buy_Amt,params)
                    if Buy_Amt*now_price_binance < float(balance_binanace['USDT']['free']):
                        print(binanceX.create_order(ticker_binance, 'market', 'sell', Buy_Amt, None, params))
                    else:
                        continue
                    if ADMoney < upbit_remain_money:
                    ...
                    line_alert.SendMessage(
                        "[김프 1단계 물] : " + str(ticker_upbit) + " " + str(round(Buy_Amt * now_price_upbit/10000, 1)) + "만원")
                    ...
                    time.sleep(0.1)
                    line_alert.SendMessage(
                        "[김프 2단계 물] : " + str(ticker_upbit) + " " + str(round(Buy_Amt * now_price_upbit/10000, 1)) + "만원")
                    ...
                    time.sleep(0.1)
                    line_alert.SendMessage(
                        "[김프 3단계 물] : " + str(ticker_upbit) + " " + str(round(Buy_Amt * now_price_upbit/10000, 1)) + "만원")
                    ...
                    time.sleep(0.1)
                    line_alert.SendMessage(
                        "[김프 4단계 물] : " + str(ticker_upbit) + " " + str(round(Buy_Amt * now_price_upbit/10000, 1)) + "만원")
